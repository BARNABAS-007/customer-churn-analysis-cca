...
X_scaled = scaler.transform(X.astype(float)) # convert to numpy float
...
with tab1:
    st.plotly_chart(px.pie(df, names="Churn Prediction"))
    if "MonthlyCharges" in df.columns:
        st.plotly_chart(px.box(df, x="Churn Prediction", y="MonthlyCharges"))
    if "tenure" in df.columns:
        st.plotly_chart(px.histogram(df, x="tenure", color="Churn Prediction"))

with tab2:
    st.dataframe(df[["Churn Prediction","Churn Probability","Risk Level","CLV"]])

with tab3:
    st.dataframe(df[["Risk Level","Churn Probability","Retention Strategy"]])

with tab4:
    if "MonthlyCharges" in df.columns and "tenure" in df.columns:
        fig = px.scatter(df, x="MonthlyCharges", y="tenure", color="Cluster")
        st.plotly_chart(fig)
...
st.dataframe(df[df["Risk Level"] == "High Risk"])

# ------------------------------------------------
# CHURN SIMULATOR (FIXED)
# ------------------------------------------------
...
st.download_button("Download Prediction Results", csv, "churn_predictions.csv")
